...
Current2 production data has a separate post-merge live smoke in `.github/workflows/current2-live-data-smoke.yml`. It runs from `main` updates, retries through normal Cloudflare deployment lag, and verifies the live portfolio API rather than mocked or build-time data. Keep this check out of the pre-merge critical path.
...
- `main` updates: trigger the separate Current2 live-data smoke, which validates the deployed production portfolio API and remains outside the pull-request merge gate

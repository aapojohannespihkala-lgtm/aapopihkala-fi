...
Current production data has a separate post-merge live smoke in `.github/workflows/current-live-data-smoke.yml`. It runs only after a successful `main` Build check, retries through normal Cloudflare deployment lag, and verifies the live portfolio API rather than mocked or build-time data. Keep this check out of the pre-merge critical path.
...
- successful `main` Build checks: trigger the separate Current live-data smoke, which validates the deployed production portfolio API and remains outside the pull-request merge gate

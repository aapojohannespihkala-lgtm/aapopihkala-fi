import { onRequestGet as getBaseMarketsResponse } from './markets';
...
const parseBankOfFinlandDaily = (html: string): Observation[] => {
  ...
  if (observations.length < 2) throw new Error('Bank of Finland daily report has too few observations');
  return observations;
};
...
const buildEuriborSeries = (
  ...
  for (const item of daily) byDate.set(item.observedAt, item);
  byDate.set(current.observedAt, { observedAt: current.observedAt, value: current.value });
...
const hasCompleteFeed = (payload: MarketsPayload) => {
  ...
  const hasEuriborItem = items.some(
    (item) => item && typeof item === 'object' && (item as { id?: unknown }).id === 'euribor-3m'
  );
...
const buildFallbackPayload = async () => {
  const [dailyHtml, monthlyCsv, worldResult] = await Promise.all([
    fetchText(EURIBOR_DAILY_HISTORY_URL, 'text/html'),
  ...
  const daily = parseBankOfFinlandDaily(dailyHtml);
  const monthly = parseEcbMonthly(monthlyCsv);
  const latest = daily.at(-1);
  if (!latest) throw new Error('Current Euribor observation is missing');

  const euribor: MarketMacroItem = { id: 'euribor-3m', ...latest };
  const series: MarketSeries[] = [buildEuriborSeries(daily, monthly, euribor)];
  ...
  return {
    items: [euribor],
    series,
    source: 'Bank of Finland daily report + ECB + Yahoo Finance',
    recovered: true,
  };
};

export const onRequestGet = async (context: { request: Request }) => {
  const baseResponse = await getBaseMarketsResponse(context);
  ...
  if (baseResponse.ok) {
    try {
      ...
      // Fall through to the independent recovery path.
    }
  }

  try {
    return jsonResponse(await buildFallbackPayload());
  } catch {

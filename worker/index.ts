...
import { onRequestGet as getEnrichedPortfolioResponse } from '../functions/api/current/portfolio-enriched';
...
const worker = {
  async fetch(request: Request, env: WorkerEnv): Promise<Response> {
    ...
    if (url.pathname === MARKETS_PATH) {
      if (request.method !== 'GET') return methodNotAllowed();
      if (url.searchParams.get('portfolio') === '1') return getEnrichedPortfolioResponse();

...
test('renders league-wide live and upcoming games with Current glyphs', async ({ page }) => {
  ...
  const schedulePelicans = schedule.locator('[data-liiga-schedule-team-id="pelicans"]');
  const standingsPelicans = page.locator('[data-liiga-row="pelicans"] [data-liiga-mark-id="pelicans"]');
  const clubsPelicans = page.locator('[data-liiga-club-mark="pelicans"] [data-liiga-mark-id="pelicans"]');
  await expect(standingsPelicans).toHaveCount(1);
  await expect(clubsPelicans).toHaveCount(1);

  const opticalScales = await Promise.all([
    schedulePelicans.evaluate((node) => getComputedStyle(node).getPropertyValue('--liiga-mark-scale').trim()),

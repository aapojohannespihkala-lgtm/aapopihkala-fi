...
const payload = (channel: 'prod' | 'dev') => {
  ...
  return {
    ...
    layouts: {
      ...
      large: hasHsl
        ? ['weather', 'electricity', 'markets', 'hsl', 'rates', 'liiga']
        : ['weather', 'electricity', 'markets', 'rates', 'liiga'],
    },
    sections: [
      {
        id: 'weather',
...
test('widget preview exposes staged HSL after switching to dev', async ({ page }) => {
  await page.route(/\/api\/current\/widget-v2\?channel=prod$/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',

...
Current app version: **2.10.59**.

The APK now exposes two deliberately separate widget providers. **Snapshot Widget** keeps the existing Honor/phone rendering unchanged. **Snapshot Tablet** uses the same cache, refresh workers and data contract but has its own large-screen composition: larger typography, a larger Electricity chart and equal-height section bands that consume the available vertical space instead of leaving the lower half empty.

...
Electricity keeps the day average as the primary value. Its compact detail rows use three full-width lines below the main value/chart row: today's LOW/HIGH first, TOMORROW AVG second, and MONTH AVG third. Tomorrow uses `--.--` before the complete day-ahead price set is available. These compact rows omit the `c/kWh` unit. In the large split layout, the numeric day average uses the same primary-value scale as the other main metrics while `c/kWh` is rendered as a smaller unit beside it. The chart occupies the same right-side grid column as Markets and HSL support content, with its visual left and right edges aligned to that column and the widget content edge. The approved bar baseline stays at the same visual height and the 50 dp chart slot keeps the same 16 dp baseline-to-bottom inset. There is no longer a global price-label band: the plot begins 2 dp from the bitmap top, so ordinary bars can use almost the full space above the fixed baseline. The current price is anchored to the current-time marker. Its full measured text width is checked against every hourly bar underneath the label, and the price floats above the highest of those bars so neighboring peaks cannot collide with the text. The renderer shrinks the price text first if the bitmap top is tight; only as a final safety fallback are bars directly under the label locally capped enough to preserve the 2 px clearance. Bars outside the label footprint keep their full normalized height. The time-axis labels remain safely inside the shortened chart bitmap while the approved bar baseline stays unchanged.

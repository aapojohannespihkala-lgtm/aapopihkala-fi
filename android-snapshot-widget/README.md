...
Tapping the bitmap refresh icon starts a manual refresh and swaps the icon to a native indeterminate `ProgressBar` until the attempt finishes. The icon includes internal transparent padding so launcher font metrics cannot clip it at the widget edge.

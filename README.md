...
Erillinen `.github/workflows/current-live-data-smoke.yml` käynnistyy onnistuneen `main`-haaran Build checkin jälkeen. Se odottaa tarvittaessa Cloudflare-deployta lyhyillä uusintayrityksillä ja tarkistaa tuotannon `/api/current/markets?portfolio=1`-vastauksesta kaikki 19 portfolio-riviä, `unavailable`-tilan sekä Nordnet- ja OP-adapterien vaaditut tuottojaksot. Live-smoke on post-merge-tarkistus eikä se hidasta tai blokkaa pull requestin mergeä.
...
GitHub Actionsin Build check ja Cloudflaren tuotantodeploy ovat eri vaiheita. CI tarkistaa repomuutoksen ja Cloudflare julkaisee tuotantoversion oman Workers Builds -integraationsa mukaisesti. Currentin erillinen live-data-smoke tarkistaa julkaistun API:n vasta Build checkin jälkeen eikä korvaa Cloudflaren deploy-mekanismia.
